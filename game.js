...
function resizeCanvas() {
    ...
    console.log( // 디버깅 로그
        "resizeCanvas called. Container rect (W, H, T, L):", 
        containerRect.width, containerRect.height, containerRect.top, containerRect.left,
        "Window (inH, inW):", window.innerHeight, window.innerWidth
    );
    ...
    if (containerRect.width > 0 && containerRect.height > 0) {
        ...
        console.warn("resizeCanvas called while gameContainer might be hidden or has no dimensions. Canvas not resized this time.");
    }
}
...
function showScreen(screenToShow) {
    console.log("showScreen called for:", screenToShow ? screenToShow.id : "a null screen"); // 디버깅 로그
    document.querySelectorAll('.screen').forEach(screen => {
        screen.classList.remove('show');
    });
    if (screenToShow) {
        screenToShow.classList.add('show');
        console.log(screenToShow.id + " classList after adding 'show':", screenToShow.classList.toString()); // 디버깅 로그
        console.log(screenToShow.id + " display style:", window.getComputedStyle(screenToShow).display); // 디버깅 로그
    } else {
...
function checkAndAwardQuests() {
    console.log("Checking quests. Current game stats:", currentGameStats); // 디버깅 로그
...
// --- 게임 루프 및 핵심 로직 (디버깅 로그 및 놓친 코인 처리 수정 포함) ---
function gameLoop() { 
    ...
    villains.forEach((villain, index) => {
        villain.update();
        // 악당(코인)을 놓쳤을 때의 로직 수정
        if (villain.y - villain.radius > canvas.height && !villain.sliced) {
            const missedVillainType = {
                isNormal: !villain.isGoldRushCoin && !villain.isLifeBonusCoin && !villain.isBombCoin && !villain.isIceCoin,
                isPositiveSpecial: villain.isGoldRushCoin || villain.isLifeBonusCoin,
                isBomb: villain.isBombCoin,
                isIce: villain.isIceCoin
            };
            
            console.log(`Villain missed. Type - Normal: ${missedVillainType.isNormal}, PositiveSpecial: ${missedVillainType.isPositiveSpecial}, Bomb: ${missedVillainType.isBomb}, Ice: ${missedVillainType.isIce}`);
            villains.splice(index, 1); 

            if (!gameOver) { 
                if (!isGoldRushActive) { 
                    if (missedVillainType.isNormal) { 
                        lives--; 
                        currentGameStats.livesLost++; 
                        livesDisplay.textContent = lives; 
                        console.log("Missed a NORMAL coin. Life lost. Current lives:", lives);
                        resetCombo(); 
                        if (lives <= 0) {
                            gameOver = true;
                        }
                    } else if (missedVillainType.isPositiveSpecial) {
                        console.log("Missed a positive special coin. No life lost. Combo reset.");
                        resetCombo();
                    } else {
                        console.log("Missed a Bomb or Ice coin. No life lost.");
                    }
                } else { 
...
function isLineIntersectingCircle(p1, p2, circleCenter, radius) { /* ... (이전과 동일, circleCenter 사용) ... */ 
...
// --- 퀘스트 목록 표시 함수 ---
function displayQuests() { /* ... (이전과 동일) ... */
    if (!questsListContainer) { console.error("questsListContainer not found!"); return; }
    questsListContainer.innerHTML = ''; 
    let hasQuests = false;
    for (const questId in quests) {
        if (quests.hasOwnProperty(questId)) {
            hasQuests = true; const quest = quests[questId];
            const questItemDiv = document.createElement('div'); questItemDiv.classList.add('quest-item');
            if (quest.isCompleted) questItemDiv.classList.add('completed');
            const nameH3 = document.createElement('h3'); nameH3.textContent = quest.name;
            const descP = document.createElement('p'); descP.textContent = quest.description;
            const rewardP = document.createElement('p'); rewardP.classList.add('reward'); rewardP.innerHTML = `Reward: ${quest.reward} 💎`;
            const statusP = document.createElement('p'); statusP.classList.add('status'); statusP.textContent = quest.isCompleted ? "✅ Completed!" : "⏳ In Progress";
            questItemDiv.appendChild(nameH3); questItemDiv.appendChild(descP); questItemDiv.appendChild(rewardP); questItemDiv.appendChild(statusP);
            questsListContainer.appendChild(questItemDiv);
        }
    }
    if (!hasQuests) questsListContainer.innerHTML = "<p>No quests available at the moment.</p>";
...
document.addEventListener('DOMContentLoaded', (event) => {
    ...
    if (questsButton) { 
        questsButton.addEventListener('click', () => {
            console.log("'Quests' button clicked!"); loadData(); displayQuests(); showScreen(questsScreen); 
        });
    }
    if (backToHomeFromQuestsButton) {
        backToHomeFromQuestsButton.addEventListener('click', () => {
            console.log("'Back to Home from Quests' button clicked!"); showScreen(mainHomeScreen);
        });
    }
